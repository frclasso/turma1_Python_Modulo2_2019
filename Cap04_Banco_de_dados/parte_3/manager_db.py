...
class ClientesDB(object):
    ...
    def imprimir_todos_clientes(self):
        ...
        print('{:>3s} {:20s} {:<5s} {:15s} {:>21s} {:14s} {:>25s} {:>15s} {:>15s}'.format(
            'id', 'nome', 'idade', 'cpf', 'email','fone', 'cidade', 'uf', 'criado_em'
        ))
        for c in lista:
            print('{:3d} {:23s} {:2} {:s} {:>25s} {:s} {:>25s} {:>15s} {:>25s}'.format(
                c[0], c[1], c[2],
                c[3], c[4], c[5],
                c[6], c[7], c[8]
            ))
...
if __name__=="__main__":
    ...
    c = ClientesDB()
    #c.criar_schema() ##ok
    #c.inserir_um_registro() ##ok
    #c.inserir_com_lista() ##ok
    #c.inserir_de_arquivo() ##ok
    #c.inserir_de_csv() ##ok
    #c.inserir_com_parametros()  ## ok
    #c.inserir_dados_randomicos() ## ok
    #c.imprimir_todos_clientes() ##ok
    #c.imprimir_sem_formatar()  ##ok
    #c.imprimir_cliente(7) ##ok
    #c.contar_clientes() ##ok
    #c.contar_clientes_por_idade() ##ok
    #c.contar_clientes_por_idade(18) ##ok
    #c.localizar_cliente_por_uf('SC') ##ok
    #c.meu_select() ##ok
    #c.meu_select("SELECT * FROM clientes WHERE uf='RN' ORDER BY nome;")  ##ok
    #c.atualizar(7)  ##ok
    #c.deletar(10) ##ok
    #c.alterar_tabela()  ##ok
    #c.table_info()  ##ok
